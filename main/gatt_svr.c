...
#define IMU_PAYLOAD_LEN 64

static uint8_t gatt_svr_chr_val[IMU_PAYLOAD_LEN];
...
void gatt_svr_set_imu_payload_and_notify(const uint8_t *data, size_t len)
{
    if (len > sizeof(gatt_svr_chr_val)) {
        len = sizeof(gatt_svr_chr_val);
    }
    memset(gatt_svr_chr_val, 0, sizeof(gatt_svr_chr_val));  
    memcpy(gatt_svr_chr_val, data, len);

    // Schedules notify/indicate to all subscribed peers (simple + works)
